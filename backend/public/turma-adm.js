...
// ===== LISTA DE CURSOS DE ADMINISTRAÇÃO =====
const cursosADM = ['ADM - JM', 'ADM - BÁSICO', 'ADM - AVANÇADO'];
    ...
    fetch(`${API_URL}/alunos`)
        ...
        .then(alunos => {
            ...
            horarios.forEach(horario => {
                // Filtra alunos matriculados em qualquer curso de Administração
                const matriculados = alunos.filter(aluno =>
                    aluno.matriculas && aluno.matriculas.some(mat =>
                        mat.curso && cursosADM.includes(mat.curso) &&
                        mat.dia === dia &&
                            ...
                            ? matriculados.map(aluno => `
                                <li>
                                    <a href="aluno.html?id=${aluno.FIELD1}" class="link-aluno">
                                        ${aluno.FIELD4} (${aluno.matriculas.find(m => cursosADM.includes(m.curso) && m.dia === dia && m.horario === horario.label).curso})
                                    </a>
                                </li>
                            `).join('')
