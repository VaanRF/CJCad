...
const cursosInformatica = ['INFO - JM', 'INFO - BÁSICO', 'INFO - AVANÇADO'];
    ...
    fetch(`${API_URL}/alunos`)
        ...
        .then(alunos => {
            ...
            horarios.forEach(horario => {
                // Filtra alunos matriculados em qualquer curso de Informática
                const matriculados = alunos.filter(aluno =>
                    aluno.matriculas && aluno.matriculas.some(mat =>
                        mat.curso && cursosInformatica.includes(mat.curso) &&
                        mat.dia === dia &&
                ...
                // Filtra alunos em lista de espera
                            ...
                            ? matriculados.map(aluno => `
                                <li>
                                    <a href="aluno.html?id=${aluno.FIELD1}" class="link-aluno">
                                        ${aluno.FIELD4} (${aluno.matriculas.find(m => cursosInformatica.includes(m.curso) && m.dia === dia && m.horario === horario.label).curso})
                                    </a>
                                </li>
                            `).join('')
